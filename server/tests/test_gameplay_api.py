...
class TestGameplayApi(unittest.TestCase):
    ...
    def test_actions_cover_all_gameplay_tracks(self) -> None:
        ...
        self.assertIn("template_id", session["campaign"]["current_mission"])
        self.assertIn("quality_score", session["campaign"]["current_mission"])
        self.assertIn("mission_history", session["campaign"])
        self.assertGreaterEqual(len(session["campaign"]["mission_history"]), 2)
